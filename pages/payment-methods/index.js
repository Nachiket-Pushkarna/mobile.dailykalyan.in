...
const PaymentMethods = () => {
  ...
  const saveBankDetails = () => {
    if (allData.confirm_ac_number != allData.ac_number) {
      return toast.error(
        "Account number and confirm account number are not the same."
      );
    }
    let payload = {
      user_id: user_id,
  ...
  const renderTypes = () => {
    ...
    return (
      <div className="flex flex-wrap justify-center space-x-2 gap-y-2 sm:space-x-4 overflow-x-auto py-4">
        {types.map((type) => (
          <motion.button
            key={type.id}
            className={`flex items-center justify-center px-4 py-2 sm:px-6 sm:py-3 rounded-lg font-semibold text-white focus:outline-none transition-colors duration-300 ${
              active === type.id
            ...
            whileTap={{ scale: 0.95 }}
          >
            <type.icon className="mr-1 sm:mr-2" size={20} />
      ...
      return (
        ...
        >
          ...
          <Inputbox
            ...
            onChange={(value) =>
              setAllData({ ...allData, google_pay_no: value })
            }
          />
          <Inputbox
      ...
      return (
        ...
        >
          <Inputbox
            ...
            onChange={(value) =>
              setAllData({ ...allData, ac_holder_name: value })
            }
          />
          <Inputbox
          ...
          <Inputbox
            ...
            onChange={(value) =>
              setAllData({ ...allData, confirm_ac_number: value })
            }
          />
          <Inputbox
            label="IFSC Code"
