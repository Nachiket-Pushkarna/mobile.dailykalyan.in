...
const Fund = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!amount || !validateAmount(Number(amount))) {
      setError(
        `Please enter a valid amount between ₹${MIN_AMOUNT} and ₹${MAX_AMOUNT}`
      );
      return;
    }

    const { user_name: name, mobile: phone } = getSetData(
      "userData",
      undefined,
      true
    );
    ...
    try {
      const initResponse = await ApiClient.InitiateRazorPay({
      ...
      if (status !== "success") {
        throw new Error(
          initResponse.data.message || "Failed to initiate transaction"
        );
      }
      ...
      setError(
        "An error occurred while initiating the transaction. Please try again."
      );
    }
  };
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-2xl shadow-2xl p-8 space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-sm text-gray-300">
            Top up your wallet to keep the game going!
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="relative">
            ...
            />
            <div className="absolute inset-y-0 left-0 pl-1 flex items-center pointer-events-none mt-8">
          ...
          <div className="grid grid-cols-4 gap-3 mt-3">
            {predefinedAmounts.map((amt) => (
              <motion.button
                ...
                className="w-full p-1 sm:p-2 bg-yellow-600 hover:bg-yellow-700 text-white font-bold rounded-lg transition-colors duration-300 text-center text-xs sm:text-base"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              >
                ₹{amt}
